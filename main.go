...
import (
	...
	"lowcode.com/backend/workflowengine"
)
...
func main() {
	...
	mux.Handle("/api/workflow/{app_id}/{workflow_id}", registerwithDB(workflowengine.WorkflowHandler))
	mux.Handle(
		"/api/{app_id}/{schema}/{table_name}",
		registerwithDB(appcrud.HandleDatabaseRequest),
	)
